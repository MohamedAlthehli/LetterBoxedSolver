...
public class LetterSquare {
    ...
    private boolean isValid(String letter, int wordNum, int charNum) {
        ...
        if (charNum == 0) {
            if (this.words[0].length() == 0) {
                return true;
            } else if (wordNum >= 1 && (lastLetter(this.words[wordNum - 1]).equals(letter))
                    && this.words[wordNum].length() == 0) {
                return true;
            } else {
                return false;
            }
        } else if (this.words[wordNum].length() >= 1 && this.onSameSide(letter, lastLetter(this.words[wordNum]))) {
            return false;
        } else if (this.alreadyUsed(this.words[wordNum] + letter)) {
            return false;
        } else if (!dictionary.hasString(this.words[wordNum] + letter)) {
            return false;
        } else {
            return true;
        }
    ...
    private boolean solveRB(int wordNum, int charNum, int maxWords) {
        /* Replace this line with your implementation of the method. */
        // System.out.println(wordNum + " " + charNum + " " +
        // Arrays.toString(this.words));
        if (wordNum >= maxWords) {
            return false;
        }

        if (dictionary.hasFullWord(this.words[wordNum]) && wordNum == (maxWords - 1) && this.allLettersUsed()) {
